...
> **OpenClaw Manager** 是一个企业级 AI 智能体管理平台，提供完整的 AI 智能体创建、配置和管理能力，支持多模型接入、多渠道集成，助力企业快速构建智能体管理平台。
...
> 5. 登录后用户可在用户中心创建和管理自己的 OpenClaw 实例
...
> ⚠️ **前置条件：** 使用渠道功能需要基于包含渠道插件的 OpenClaw 镜像。默认镜像不包含渠道插件，需要重新构建 Docker 镜像。可参考以下 Dockerfile 示例：
>
> ```dockerfile
...
> ```
>
> 构建完成后，将新镜像推送到镜像仓库，并在配置模板中更新镜像地址即可。
...
**A:** 用户登录页始终支持**邮箱密码登录**（页面底部），无需额外配置。如果还需要 OAuth 或 SSO 登录方式，管理员需要完成以下配置：
...
4. 判断sandboxset中写入配置是否正确，非root用户请确保为/home/node/.openclaw/openclaw.json路径
