...
- [3. 管理员功能](#3-管理员功能)
  - [3.1 仪表盘](#31-仪表盘)
  - [3.2 用户管理](#32-用户管理)
  - [3.3 OAuth 配置](#33-oauth-配置)
  - [3.4 SAML SSO 配置](#34-saml-sso-配置)
  - [3.5 模型配置](#35-模型配置)
    - [3.5.1 模型提供商](#351-模型提供商)
    - [3.5.2 配置模型提供商](#352-配置模型提供商)
    - [3.5.3 添加模型](#353-添加模型)
  - [3.6 AI 网关](#36-ai-网关)
    - [3.6.1 前置条件：创建阿里云 AI 网关](#361-前置条件创建阿里云-ai-网关)
    - [3.6.2 在管理平台中配置 AI 网关](#362-在管理平台中配置-ai-网关)
    - [3.6.3 AI 网关启用后的效果](#363-ai-网关启用后的效果)
  - [3.7 Token 统计与限流](#37-token-统计与限流)
    - [3.7.1 Token 统计](#371-token-统计)
    - [3.7.2 全局限流配置](#372-全局限流配置)
    - [3.7.3 个人限流配置](#373-个人限流配置)
  - [3.8 实例列表（管理员视图）](#38-实例列表管理员视图)
  - [3.9 渠道配置](#39-渠道配置)
  - [3.10 配置模板](#310-配置模板)
  - [3.11 技能配置](#311-技能配置)
- [4. 用户功能](#4-用户功能)
  - [4.1 实例列表](#41-实例列表)
  - [4.2 创建 OpenClaw 实例](#42-创建-openclaw-实例)
  - [4.3 实例详情与配置](#43-实例详情与配置)
  - [4.4 暂停与重启实例](#44-暂停与重启实例)
  - [4.5 用户侧 Token 用量概览](#45-用户侧-token-用量概览)
...
> **注意：** 如果页面显示"暂未配置登录方式"，说明管理员尚未在 Supabase 侧配置 OAuth 或在管理后台配置 SAML SSO。请参考 [3.3 OAuth 配置](#33-oauth-配置) 和 [3.4 SAML SSO 配置](#34-saml-sso-配置) 完成配置。
...
如需添加新的模型提供商（如 OpenAI、DeepSeek 等），需先在「配置模板」页面中的 OpenClaw 配置模板 JSON 文件里添加对应的 provider 节点（详见 [3.10 配置模板](#310-配置模板)）。

#### 3.5.2 配置模型提供商

> 如果需要添加系统默认未包含的提供商，请先参考 [3.10 配置模板](#310-配置模板) 完成配置模板管理后，再回到此步骤。
...
- **Token 限流** — 可对用户进行全局或个人级别的 Token 使用限制（详见 [3.7 Token 统计与限流](#37-token-统计与限流)）
...
- **配置 OAuth**：在 Supabase 控制台的 Authentication → Providers 中启用 OAuth 提供商（如阿里云），并在对应平台创建 OAuth 应用，将回调地址设为 `https://<Supabase URL>/auth/v1/callback`。详见 [3.3 OAuth 配置](#33-oauth-配置)。
- **或配置 SAML SSO**：在管理后台的 SAML SSO 页面添加企业 SSO 配置（如阿里云 IDaaS）。详见 [3.4 SAML SSO 配置](#34-saml-sso-配置)。
